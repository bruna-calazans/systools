...
from linear_regression_plot import plot_regression, open_html, html_abas
...
def fit_model(x, y, intercept=False, cov_type='nonrobust', plot=False):
    ...
    model_out, dfR, xx, yy, grafico = find_outliers(model, x, y, intercept, cov_type, plot)
#    html_abas(html, htmlout,'')
#    _,reg_str = plot_regression(model, x, y, intercept)
#    _,regout_str = plot_regression(model_out, xx, yy, intercept)
# TODO entender porque tem hora que essas 3 linhas acima fazem o código rodar e outras horas fazem o codigo nao rodar   
    if plot: 
        print('R²=',model.rsquared)
        #print(model.summary())
        htmlFileName,htmlStrReg = plot_regression(model, x, y, intercept)
#        try: open_html(htmlFileName)
#        except: pass
    
        if model_out is not None: 
            htmlFileOutName,htmlStroutReg = plot_regression(model_out, xx, yy, intercept)
#            try: open_html(htmlFileOutName)
#            except: pass
            html_abas('ABAS_'+ htmlFileName,htmlStrReg,htmlStroutReg,grafico)
            try: open_html('ABAS_'+ htmlFileName)
            except: pass
    #TODO alterar plot_regression para receber dois modelos simultaneos
    # plot_regression(models=[model,model__out],x=[x,xx],y=[y,yy])
...
def loop_models(df, Xcols, Ycol, mask=None, keepAll=False, CUT_R=0.5, force_intercept=False, kargs={}):
    ...
    comb = get_combinations(Xcols)
    corr_pairs = get_correlated_pairs(df, Xcols)
    comb = drop_combinations(comb, corr_pairs, Xcols)
    ...
    for c in tqdm(comb, position=0, leave=True, desc='Making regressions...'):
        ...
        if model.rsquared > CUT_R:
            df_regs=save_model(model, df_regs, intercept, test_conditions(model))
            df_regs=save_model(model_out, df_regs, intercept, test_conditions(model_out))
